...
int WebRtcSpl_AutoCorrelation(const int16_t* in_vector,
  ...
  // Find the maximum absolute value of the samples.
    int absolute = 0;
    if (in_vector == NULL || in_vector_length <= 0) {
        smax = -1;
    } else {
        for (int i = 0; i < in_vector_length; i ++) {
            absolute = abs((int)in_vector[i]);
            if (absolute > smax) {
                smax = absolute;
            }
        }
        if (smax > WEBRTC_SPL_WORD16_MAX) {
            smax = WEBRTC_SPL_WORD16_MAX;
        }
    }
