...
void WebRtcIlbcfix_CbSearch(
  ...
  //temp1 = WebRtcSpl_MaxAbsValueW16(buf, (int16_t)lMem);
                                ...
                                int16_t* vectorCustom = buf;
                                int lengthCustom = (int16_t)lMem;
                                ...
                                if (vectorCustom == NULL || lengthCustom <= 0) {
                                    ...
                                    if (temp1 > WEBRTC_SPL_WORD16_MAX) {
                                        temp1 = WEBRTC_SPL_WORD16_MAX;
                                    }
                                }
  ...
  //temp2 = WebRtcSpl_MaxAbsValueW16(target, (int16_t)lTarget);
                                ...
                                temp2 = 0;
                                int absoluteSencond = 0;
                                
                                int16_t* vectorCustomSencond = target;
                                int lengthCustomSecond = (int16_t)lTarget;
                                
                                if (vectorCustomSencond == NULL || lengthCustomSecond <= 0) {
                                    temp2 = -1;
                                } else {
                                    for (int j = 0; j < lengthCustomSecond; j ++) {
                                        absoluteSencond = abs((int)vectorCustomSencond[j]);
                                        if (absoluteSencond > temp2) {
                                            temp2 = absoluteSencond;
                                        }
                                    }
                                    if (temp2 > WEBRTC_SPL_WORD16_MAX) {
                                        temp2 = WEBRTC_SPL_WORD16_MAX;
                                    }
                                }
  ...
  for (stage=0; stage<CB_NSTAGES; stage++) {
    ...
    //WebRtcSpl_CrossCorrelation(cDotPtr, target, cb_vecPtr, lTarget, range, scale, -1);
      ...
      int32_t* cross_correlation = cDotPtr;
      int16_t* seq1 = target;
      int16_t* seq2 = cb_vecPtr;
      int16_t dim_seq = lTarget;
      int16_t dim_cross_correlation = range;
      int16_t right_shifts = scale;
      int16_t step_seq2 = -1;
      int i__ = 0, j__ = 0;
      for (i__ = 0; i__ < dim_cross_correlation; i__ ++) {
          *cross_correlation = 0;
          for (j__ = 0; j__ < dim_seq; j__ ++) {
              *cross_correlation += (seq1[j__] * seq2[step_seq2 * i__ + j__]) >> right_shifts;
          }
          cross_correlation++;
      }
...
*/
    ...
    if (lTarget==SUBL) {
      ...
      //WebRtcSpl_CrossCorrelation(cDotPtr, target, cb_vecPtr, lTarget, (int16_t)(eInd-i+1), scale, -1);
        ...
        int16_t* seq2 = cb_vecPtr;
        int16_t dim_seq = lTarget;
        int16_t dim_cross_correlation = (int16_t)(eInd-i+1);
        int16_t right_shifts = scale;
        int16_t step_seq2 = -1;
        int i__ = 0, j__ = 0;
        for (i__ = 0; i__ < dim_cross_correlation; i__ ++) {
      ...
      //WebRtcSpl_CrossCorrelation(cDotPtr, target, cb_vecPtr, lTarget, (int16_t)(eInd-sInd+1), scale, -1);
        ...
        int16_t* seq2 = cb_vecPtr;
        int16_t dim_seq = lTarget;
        int16_t dim_cross_correlation = (int16_t)(eInd-sInd+1);
        int16_t right_shifts = scale;
        int16_t step_seq2 = -1;
        int i__ = 0, j__ = 0;
        for (i__ = 0; i__ < dim_cross_correlation; i__ ++) {
