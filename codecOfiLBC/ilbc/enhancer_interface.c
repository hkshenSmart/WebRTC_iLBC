...
int WebRtcIlbcfix_EnhancerInterface( /* (o) Estimated lag in end of in[] */
  ...
  k=WebRtcSpl_DownsampleFast(
      ...
      DELAY_DS);
         */
                                            if (1) {
                                                int16_t* data_in = enh_buf+ENH_BUFL-inLen;
                                                int data_in_length = (int16_t)(inLen+ENH_BUFL_FILTEROVERHEAD);
                                                int16_t* data_out = downsampled;
                                                int data_out_length = (int16_t)WEBRTC_SPL_RSHIFT_W16(inLen, 1);
                                                int16_t* __restrict coefficients = (int16_t*)WebRtcIlbcfix_kLpFiltCoefs;
                                                int coefficients_length = FILTERORDER_DS_PLUS1;
                                                int factor = FACTOR_DS;
                                                int delay = DELAY_DS;
  ...
  for(iblock = 0; iblock<new_blocks; iblock++){
    ...
    //max16=WebRtcSpl_MaxAbsValueW16(&regressor[-50],
      ...
      if (1) {
          ...
          int16_t* vectorCustom = &regressor[-50];
          int lengthCustom = (int16_t)(ENH_BLOCKL_HALF+50-1);
          
          if (vectorCustom == NULL || lengthCustom <= 0) {
              max16 = -1;
          } else {
              for (int i = 0; i < lengthCustom; i ++) {
                  absolute = abs((int)vectorCustom[i]);
                  if (absolute > max16) {
                      max16 = absolute;
                  }
              }
              if (max16 > WEBRTC_SPL_WORD16_MAX) {
                  max16 = WEBRTC_SPL_WORD16_MAX;
              }
          }
      }
    ...
    //WebRtcSpl_CrossCorrelation(corr32, target, regressor,
      ...
      if (1) {
          int32_t* cross_correlation = corr32;
          int16_t* seq1 = target;
          int16_t* seq2 = regressor;
          int16_t dim_seq = ENH_BLOCKL_HALF;
          int16_t dim_cross_correlation = 50;
          int16_t right_shifts = (int16_t)shifts;
          int16_t step_seq2 = -1;
    ...
    */
    ...
    //max16=WebRtcSpl_MaxAbsValueW16(regressor, (int16_t)(plc_blockl+3-1));
      ...
      if (1) {
          ...
          int16_t* vectorCustom = regressor;
          int lengthCustom = (int16_t)(plc_blockl+3-1);
    ...
    //WebRtcSpl_CrossCorrelation(corr32, target, regressor,
      ...
      if (1) {
          ...
          int16_t* seq1 = target;
          int16_t* seq2 = regressor;
          int16_t dim_seq = plc_blockl;
          int16_t dim_cross_correlation = 3;
          int16_t right_shifts = (int16_t)shifts;
          int16_t step_seq2 = 1;
      ...
      */
      ...
      //max=WebRtcSpl_MaxAbsValueW16(
        ...
        if (1) {
            ...
            int16_t* vectorCustom = &enh_buf[ENH_BUFL-iLBCdec_inst->blockl-plc_blockl];
            int lengthCustom = plc_blockl;
            ...
            if (vectorCustom == NULL || lengthCustom <= 0) {
                ...
                if (max > WEBRTC_SPL_WORD16_MAX) {
                    max = WEBRTC_SPL_WORD16_MAX;
                }
            }
        }
