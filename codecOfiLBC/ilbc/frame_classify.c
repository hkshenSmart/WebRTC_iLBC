...
int16_t WebRtcIlbcfix_FrameClassify(
  ...
  */
  ...
  //max = WebRtcSpl_MaxAbsValueW16(residualFIX, iLBCenc_inst->blockl);
                                                    ...
                                                    int absolute = 0;
                                                    max = 0;
                                                    if (residualFIX == NULL || iLBCenc_inst->blockl <= 0) {
                                                        max = -1;
                                                    } else {
                                                        for (int i = 0; i < iLBCenc_inst->blockl; i ++) {
                                                            absolute = abs((int)residualFIX[i]);
                                                            if (absolute > max) {
                                                                max = absolute;
                                                            }
                                                        }
  ...
  //maxW32 = WebRtcSpl_MaxValueW32(ssqEn, (int16_t)(iLBCenc_inst->nsub-1));
                                                    ...
                                                    maxW32 = WEBRTC_SPL_WORD32_MIN;
                                                    if ((int32_t *)ssqEn == NULL || (int16_t)(iLBCenc_inst->nsub - 1) <= 0) {
                                                        
                                                    } else {
                                                        for (int i = 0; i < (int16_t)(iLBCenc_inst->nsub - 1); i ++) {
                                                            if (ssqEn[i] > maxW32) {
                                                                maxW32 = ssqEn[i];
                                                            }
                                                        }
