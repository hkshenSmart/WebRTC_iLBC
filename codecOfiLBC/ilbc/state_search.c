...
void WebRtcIlbcfix_StateSearch(
  ...
  //max = WebRtcSpl_MaxAbsValueW16(residual, iLBCenc_inst->state_short_len);
                                   ...
                                   int16_t* vectorCustom = residual;
                                   int lengthCustom = iLBCenc_inst->state_short_len;
  ...
  //maxVal=WebRtcSpl_MaxAbsValueW16(sampleAr, iLBCenc_inst->state_short_len);
                                   ...
                                   int16_t* vectorCustomSencond = sampleAr;
                                   int lengthCustomSecond = iLBCenc_inst->state_short_len;
                                   ...
                                   if (vectorCustomSencond == NULL || lengthCustomSecond <= 0) {
                                       ...
                                       for (int j = 0; j < lengthCustomSecond; j ++) {
                                           ...
                                           if (absoluteSencond > maxVal) {
                                               maxVal = absoluteSencond;
                                           }
                                       }
                                       if (maxVal > WEBRTC_SPL_WORD16_MAX) {
                                           maxVal = WEBRTC_SPL_WORD16_MAX;
                                       }
                                   }
